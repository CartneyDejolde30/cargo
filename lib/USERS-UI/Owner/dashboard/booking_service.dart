...
class BookingService {
  ...
  Future<List<Booking>> fetchRecentBookings(String ownerId, {int limit = 5}) async {
    try {
      final url = Uri.parse("${ApiConstants.baseUrl}api/dashboard/recent_bookings.php?owner_id=$ownerId&limit=$limit");
  ...
  Future<List<Booking>> fetchUpcomingBookings(String ownerId) async {
    try {
      final url = Uri.parse("${ApiConstants.baseUrl}api/dashboard/upcoming_bookings.php?owner_id=$ownerId");
      final response = await http.get(url).timeout(ApiConstants.apiTimeout);
