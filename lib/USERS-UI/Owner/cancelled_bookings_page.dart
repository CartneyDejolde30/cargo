...
class CancelledBookingsPage extends StatefulWidget {
  ...
  const CancelledBookingsPage({super.key, required this.ownerId});
...
class _CancelledBookingsPageState extends State<CancelledBookingsPage> {
  ...
  Future<void> _fetchCancelledBookings() async {
    ...
    try {
      final bookings = await _bookingService.fetchCancelledBookings(widget.ownerId);
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Failed to load cancelled bookings';
        });
      }
    }
  }
  ...
  Widget _buildContent() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton.icon(
              onPressed: _fetchCancelledBookings,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Bookings cancelled by renters will appear here',
    ...
    return RefreshIndicator(
      onRefresh: _fetchCancelledBookings,
      color: Colors.orange.shade700,
      child: ListView.builder(
        padding: const EdgeInsets.all(20),
        itemCount: _cancelledBookings.length,
        itemBuilder: (context, index) {
          return _buildBookingCard(_cancelledBookings[index]);
        },
      ),
    );
  }
